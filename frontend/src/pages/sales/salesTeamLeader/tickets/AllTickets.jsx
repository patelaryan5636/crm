...
import {
  kpiTickets, MY_TICKETS_SEED, ROLE_TL, ROLE_EXEC, ROLE_MANAGER,
} from "../ticketsStore";
import {
...
// Team Tickets columns — executives raised to TL
const ticketCols = [
...
export default function AllTickets({ tickets, setTickets }) {
  ...
  const openMyView = (row) => {
    ...
    openModal("tl-myticket-view");
  };

  // ── Send reply (team tickets) — TL replying to an executive ──────────────
  const sendReply = (msg) => {
    setTickets((prev) =>
      prev.map((t) =>
        t.id === selected.id
  ...
  // ── Send reply (my tickets) — TL adding to their own ticket thread ───────
  ...
  // ── Submit new ticket — TL creates a ticket addressed to Sales Manager ───
  ...
  // ── Live KPI counts (combined: team + my tickets) ────────────────────────
  ...
  const actions = [
    ...
    {
      icon: <AlertTriangle size={15} />, tooltip: "Escalate to Manager",
      variant: "danger",
      onClick: (row) => setTickets((prev) => prev.map((t) =>
        t.id === row.id ? { ...t, status: "Escalated", raisedTo: ROLE_MANAGER } : t
      )),
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* ── My Tickets table (TL → Sales Manager) ────────────────────────── */}
      <DataTable
        title="My Tickets"
      ...
      {/* ── Team Tickets table (Executives → TL) ─────────────────────────── */}
      <DataTable
        title="Team Tickets"
        columns={ticketCols}
        rows={tickets}
        ...
        <div className="flex flex-col gap-5">
          <Grid cols={12} gap={4}>
            ...
            <div className="col-span-12">
              <DataField
                label="Ticket Title *" id="tl-tkt-title" size={12}
                value={form.title}
            ...
            <div className="col-span-6 flex flex-col gap-1.5">
              ...
              <Select value={form.category} onChange={(e) => setField("category", e.target.value)} placeholder="Select category" size={12}>
                ...
                <Option value="Executive Issue" label="Executive Issue" />
            ...
            <div className="col-span-12">
              <DataField
                label="Send To" id="tl-tkt-assign" size={12}
                value={form.assignedTo || ROLE_MANAGER}
            ...
            <div className="col-span-12">
              <DataField
                label="Description *" id="tl-tkt-desc" type="textarea" rows={4} size={12}
      ...
      <Modal id="tl-ticket-view" title="Ticket Details" size="lg">
        {selected && (
          <TicketDetailContent
            selected={selected}
            onSendMsg={sendReply}
            onClose={() => closeModal("tl-ticket-view")}
          />
        )}
      </Modal>

      {/* ── View / Reply Modal (My Tickets) ──────────────────────────────── */}
      <Modal id="tl-myticket-view" title="My Ticket Details" size="lg">
        {mySelected && (
          <TicketDetailContent
            selected={mySelected}
            onSendMsg={sendMyReply}
            onClose={() => closeModal("tl-myticket-view")}
          />
        )}
      </Modal>
