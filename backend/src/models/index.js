...
const UserSchema = new Schema(
  {
    ...
    prereqStep: {
      type: String,
      enum: ["password", "bank-details", "completed"],
      default: "password",
    },
    onboardingAudit: [
      {
...
const WebhookLogSchema = new Schema(
  {
    ...
    rawBody: { type: String, default: null }, // raw request body string for debugging
    signature: String,
    isVerified: { type: Boolean, default: false },
...
const BulkLeadUploadSchema = new Schema(
  {
    ...
    status: {
      type: String,
      enum: ["PROCESSING", "PREVIEWED", "DONE", "PARTIAL", "FAILED"],
      default: "PROCESSING",
    },
    assignedTo: { type: Schema.Types.ObjectId, ref: "User", default: null },
  },
...
// After Finance Manager approves → pushed to Management Manager as a project.
// ════════════════════════════════════════════════════════════
const WorkOrderSchema = new Schema(
  {
    ...
    // Management handoff — set when Finance approves
    sentToManagement: { type: Boolean, default: false },
    sentToManagementAt: { type: Date, default: null },
  },
  { timestamps: true },
);

WorkOrderSchema.index({ admin: 1, project: 1 });
WorkOrderSchema.index({ admin: 1, prospectForm: 1 });
WorkOrderSchema.index({ admin: 1, approvalStatus: 1 });
WorkOrderSchema.index({ admin: 1, sentToManagement: 1 });

// ════════════════════════════════════════════════════════════
// MODEL 32A — WORK ORDER COUNTER
// Atomic sequence number per admin for WO numbers.
// ════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════
// MODEL 34A — EXPENSE COUNTER
// Atomic sequence number per admin for expense IDs.
// ════════════════════════════════════════════════════════════
...
ExpenseSchema.index({ admin: 1, expenseDate: -1, isDeleted: 1 });
ExpenseSchema.index({ admin: 1, category: 1 });
ExpenseSchema.index({ admin: 1, status: 1 });
ExpenseSchema.index({ admin: 1, expenseId: 1 }, { unique: true });
...
ExpenseSchema.pre("validate", async function () {
  ...
  const counter = await mongoose.model("ExpenseCounter").findOneAndUpdate(
    { admin: this.admin },
    { $inc: { seq: 1 } },
    {
      upsert: true,
  ...
  this.expenseId = `${counter.prefix}-${String(counter.seq).padStart(6, "0")}`;
});
...
const AnnouncementSchema = new Schema(
  {
    ...
    type: {
      type: String,
      enum: ["INFO", "ANNOUNCEMENT", "WARNING", "APPRECIATION"],
      required: true,
    },
...
module.exports = {
  ...
  SuperAdminLoginLog: mongoose.model(
    "SuperAdminLoginLog",
  ...
  EmailVerification: mongoose.model(
    "EmailVerification",
  ...
  UserLimitOverride: mongoose.model(
    "UserLimitOverride",
    UserLimitOverrideSchema,
  ),
  DataLimitOverride: mongoose.model(
    "DataLimitOverride",
    DataLimitOverrideSchema,
  ),
  ...
  LeadAssignmentHistory: mongoose.model(
  ...
  WoCounter: mongoose.model("WoCounter", WoCounterSchema),
  Invoice: mongoose.model("Invoice", InvoiceSchema),
  Expense: mongoose.model("Expense", ExpenseSchema),
  ExpenseCounter: mongoose.model("ExpenseCounter", ExpenseCounterSchema),
