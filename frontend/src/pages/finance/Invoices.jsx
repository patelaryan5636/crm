...
const initialInvoices = [
  ...
  },
  {
    ...
    status: "Paid",
    date: "2025-07-12",
    notes: "Logo + brand kit.",
  },
];

const calcGst = (amount, gstPct) => Math.round((amount * gstPct) / 100);
const calcTotal = (amount, gstPct, discount) =>
  amount + calcGst(amount, gstPct) - discount;
...
export default function Invoices() {
  ...
  const totalAmt = invoices.reduce(
    (acc, i) => acc + calcTotal(i.amount, i.gstPct, i.discount),
    0,
  );
  ...
  const saveEdit = () => {
    ...
    setInvoices((prev) =>
      prev.map((i) =>
        i.id === selected.id
          ? { ...i, ...editForm, amount: amt, gstPct: gst, discount: disc }
          : i,
      ),
    );
    closeModal("inv-edit");
  ...
  const columns = [
    ...
    { key: "gstPct", label: "GST", render: (v) => `${v}%` },
    {
      ...
      label: "Total",
      render: (_, row) =>
        `₹${calcTotal(row.amount, row.gstPct, row.discount).toLocaleString()}`,
    },
    {
      key: "status",
      label: "status",
      ...
      <DataTable
        ...
        actions={[
          ...
          },
          {
            ...
            variant: "ghost",
            onClick: (row) => downloadInvoicePDF(row),
          },
          {
            icon: <Send size={15} />,
            ...
            variant: "success",
            onClick: (row) =>
              showToast(`✅ Invoice ${row.id} sent to ${row.email}`),
          },
        ]}
      />
            ...
            return (
              <div className="flex flex-col gap-4">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-white/70 mt-1">
                        123 Business Park, Mumbai · billing@yourcrm.in
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full mt-1 inline-block ${statusColor(selected.status)}`}
                      >
                        {selected.status}
                      </span>
                    </div>
                ...
                <ModalGrid title="Services / Items" cols={2}>
                  ...
                  <ModalData
                    label="Discount"
                    value={`₹${selected.discount.toLocaleString()}`}
                  />
                </ModalGrid>
                <div className="bg-[#2a465a]/5 border border-[#2a465a]/20 rounded-2xl px-4 py-3 flex justify-between items-center">
                  ...
                  <span className="text-xl font-black text-[#2a465a]">
                ...
                <ModalGrid title="Terms" cols={1}>
                  <ModalData
                    label="Terms"
                    value="Payment due within 15 days. Late payments incur 1.5% monthly interest."
                  />
                </ModalGrid>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DataField
                ...
                onChange={(e) =>
                  setEditForm((p) => ({ ...p, client: e.target.value }))
                }
                size={12}
              />
              <DataField
                ...
                value={editForm.item}
                onChange={(e) =>
                  setEditForm((p) => ({ ...p, item: e.target.value }))
                }
                size={12}
              />
              <DataField
              ...
              <DataField
                ...
                id="inv-gst"
                type="number"
                value={editForm.gstPct}
                onChange={(e) =>
                  setEditForm((p) => ({ ...p, gstPct: e.target.value }))
                }
                size={12}
              />
              <DataField
