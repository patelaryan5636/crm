/**
 * Finance Manager — Support Tickets Page
 * (Modeled on the Sales Manager support panel)
 *
 *  My Tickets       : tickets the Finance Manager raised (route to Admin / Graphura)
 *  Assigned Tickets : tickets escalated up from Finance Executives (FM replies / resolves)
 *
 * Finance is a single-role department, so the Assigned table is typically empty —
 * it is kept for parity with the hierarchy (FINANCE_EXECUTIVE → FINANCE_MANAGER → ADMIN).
 */
...
import { Ticket, AlertCircle, Clock, CheckCircle, MessageSquare, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function FinanceSupport() {
  ...
  const handleEscalate = async (row) => {
    setConfirmData({
      message: "Are you sure you want to escalate this ticket to Admin?",
      confirmText: "Escalate",
      confirmVariant: "danger",
      onConfirm: async () => {
        try {
          await escalateTicket(row._id, 'Escalated by Finance Manager to Admin');
          await fetchData();
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      await createTicket({
        ...
        targetHierarchy: 'ADMIN',
      });
      setForm(blankForm);
      setFormErr({});
      closeModal('fm-create-ticket-modal');
      await fetchData();
    } catch (err) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-end">
        <Button text="+ Create Ticket" variant="primary" size={3}
          onClick={() => openModal('fm-create-ticket-modal')} />
      </div>
      ...
      <DataTable title="Assigned Tickets" columns={allCols} rows={allTickets} actions={allActions}
      ...
      {/* Create Ticket Modal */}
      <Modal id="fm-create-ticket-modal" title="Create New Ticket" size="lg">
        <div className="flex flex-col gap-5">
          <Grid cols={12} gap={4}>
            <div className="col-span-12">
              <DataField label="Ticket Title *" id="fm-tkt-title" size={12}
            ...
            <div className="col-span-12">
              <DataField label="Description *" id="fm-tkt-desc" type="textarea" rows={4} size={12}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            <Button text="Cancel" variant="secondary" size={3}
              onClick={() => { setForm(blankForm); setFormErr({}); closeModal('fm-create-ticket-modal'); }} />
      ...
      {/* Assigned Ticket View/Reply Modal */}
      <Modal id="fm-all-ticket-view" title="Ticket Details" size="lg">
        {selected && (
          <ReplyTicketContent selected={selected} onSendMsg={sendAllReply}
            onClose={() => closeModal('fm-all-ticket-view')}
            loading={replyLoading} currentUser={CURRENT_USER} />
        )}
      </Modal>

      {/* My Ticket View Modal (read-only) */}
      <Modal id="fm-my-ticket-view" title="My Ticket Details" size="lg">
        {mySelected && (
          <ReadOnlyTicketContent selected={mySelected}
            onClose={() => closeModal('fm-my-ticket-view')} currentUser={CURRENT_USER} />
        )}
      </Modal>
...
function ReplyTicketContent({ selected, onSendMsg, onClose, loading, currentUser }) {
  ...
  const conversation = (() => {
    ...
    return [{ id: '__initial__', sender: selected.raisedBy || 'Finance Executive',
      time: selected.createdDate ? `${selected.createdDate} 00:00` : '', text: selected.description }, ...msgs];
  })();
  const hasReplied = conversation.some(m => m.sender === currentUser);
