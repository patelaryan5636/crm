/**
 * ManagementTLAnnouncements.jsx
 * Team Communication page for the Management Team Leader.
 * Same design as Sales Team Leader announcements.
 * TL can send to: Team (all their employees) or Employee (individual).
 * Uses the same announcement backend API.
 */
...
// ── Type badge styles (same as Sales TL) ─────────────────────────────────────
const TYPE_STYLE = {
...
const getTLName = () => {
  try {
    ...
    return u?.name || "Management Team Leader";
  } catch { return "Management Team Leader"; }
};

export default function ManagementTLAnnouncements() {
  ...
  useEffect(() => {
    fetchAnnouncementMeta()
      .then(({ messageTypes: mt, audienceOptions: ao }) => {
        ...
        setAudienceOptions(ao); // ["Team", "Employee"] for Management TL
      })
      .catch(() => {
        setMessageTypes(["Announcement", "Warning", "Appreciation"]);
        setAudienceOptions(["Team", "Employee"]);
      })
  ...
  const actions = [
    {
      ...
      onClick: (row) => { setViewRow(row); openModal("mgmt-tl-ann-view"); },
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
        ...
        <Grid cols={12} gap={4}>
          ...
          <div className="col-span-12 sm:col-span-6">
            <SelectField label="Message Type *" id="mgmt-tl-type" size={12}
          ...
          {/* Send To */}
          <div className="col-span-12 sm:col-span-6">
            <SelectField label="Send To *" id="mgmt-tl-audience" size={12}
          ...
          <div className="col-span-12 sm:col-span-6">
            <DatePicker
              label="Expiry Date (optional)" id="mgmt-tl-expiry"
          ...
          <div className="col-span-12">
            <DataField label="Title *" id="mgmt-tl-title" size={12}
              value={form.title}
          ...
          {/* Message body */}
          <div className="col-span-12">
            <DataField label="Message *" id="mgmt-tl-body" type="textarea" rows={4} size={12}
              value={form.body}
            ...
            <div className="col-span-12">
              ...
              <div className={`rounded-2xl border p-5 space-y-3 transition-all ${TYPE_STYLE[form.type] || "bg-blue-50 border-blue-200"}`}>
                ...
                <p className="text-[10px] text-slate-400 pt-1">
                  From: {tlName} (Management Team Leader) &nbsp;·&nbsp; {new Date().toLocaleDateString("en-IN", { day:"2-digit", month:"short", year:"numeric" })}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="col-span-6">
            <Button text={submitting ? "Sending…" : "Send Message"} variant="primary"
      ...
      <DataTable
        ...
        actions={actions}
        size={12} pageSize={10} searchable exportable exportFileName="mgmt_tl_messages"
        filters={[
          { title: "Type",     type: "toggle", key: "type",     options: ["Warning","Appreciation","Announcement"] },
          { title: "Audience", type: "toggle", key: "audience", options: ["Team","Employee"] },
          { title: "Status",   type: "toggle", key: "status",   options: ["Active","Expired"] },
        ]}
      />

      {/* View Detail Modal */}
      <Modal id="mgmt-tl-ann-view" title="Message Details" size="xl">
        {viewRow && (
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100">
              <Button text="Close" variant="ghost" size={3} onClick={() => closeModal("mgmt-tl-ann-view")} />
            </div>
          </div>
        )}
      </Modal>
