...
const transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: process.env.SMTP_USER || process.env.BREVO_SENDER_EMAIL || "patelaryan5636@gmail.com",
  ...
  try {
    ...
    if (payload.pdfPath) {
      ...
      if (payload.pdfPath.startsWith("http")) {
        try {
          const pdfResponse = await axios.get(payload.pdfPath, {
          ...
          logger.info(
            `Sending email with downloaded base64 attachment: ${fileName}`,
          );
        } catch (downloadErr) {
          logger.error(
            `Failed to download attachment PDF from URL: ${payload.pdfPath}`,
            downloadErr.message,
          );
    ...
    logger.info(
      `Prospect quotation email sent to ${payload.email} (${messageId || "no message id"})`,
    );
    ...
    throw new Error(
      typeof details === "string"
        ? details
        : "Unable to send prospect quotation email. Please try again.",
    );
  }
};
...
const sendRazorpayLinkEmail = async (payload) => {
  try {
    ...
    const messageId = response.data.messageId || response.data.id || null;
    logger.info(
      `Razorpay link email sent to ${payload.email} (${messageId || "no message id"})`,
    );
    return { success: true, messageId };
  } catch (error) {
    const details =
    ...
    throw new Error(
      typeof details === "string"
        ? details
        : "Unable to send Razorpay payment link email. Please try again.",
    );
  }
};
...
const sendInvoiceEmail = async (payload) => {
  try {
    ...
    const statusColor =
      status === "PAID" ? "#065f46" : status === "SENT" ? "#1e40af" : "#92400e";
    const statusBg =
      status === "PAID" ? "#d1fae5" : status === "SENT" ? "#dbeafe" : "#fef3c7";
    ...
    throw new Error(
      typeof details === "string" ? details : "Unable to send invoice email",
    );
  }
};
  ...
  <div style="padding:24px 32px;margin-top:24px;border-top:1px solid #e2e8f0;">
    ...
    if (termsPdfUrl) {
      ...
      if (termsPdfUrl.startsWith("http")) {
        try {
          const pdfResponse = await axios.get(termsPdfUrl, {
            responseType: "arraybuffer",
            timeout: 10000,
          });
          const b64 = Buffer.from(pdfResponse.data).toString("base64");
          emailPayload.attachment = [{ content: b64, name: fileName }];
          logger.info(
            `Sending work order email with downloaded base64 attachment: ${fileName}`,
          );
        } catch (downloadErr) {
          logger.error(
            `Failed to download work order PDF from URL: ${termsPdfUrl}`,
            downloadErr.message,
          );
    ...
    throw new Error(
      typeof details === "string" ? details : "Unable to send work order email",
    );
  }
};
...
<div style="max-width:600px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
  ...
  <div style="padding:20px 32px;border-top:1px solid #e2e8f0;text-align:center;">
    ...
    throw new Error(
      typeof details === "string"
        ? details
        : "Unable to send tracking link email.",
    );
  }
};

/**
 * Send Contact Us Acknowledgement Email
  ...
  try {
    ...
    const response = await axios.post(
      "https://api.brevo.com/v3/smtp/email",
      {
        ...
        subject: "Thank You for Contacting Graphura CRM",
        htmlContent: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2>Thank You for Contacting Us</h2>
            <p>Hi ${name},</p>
            <p>Thank you for contacting Graphura CRM.</p>
            <p>We have successfully received your query and our team will review it shortly.</p>
            <p>A member of our team will reach out to you within 24 hours.</p>
            <p>If your request is urgent, please reply directly to this email.</p>
            <br />
            <p>Regards,<br/>Graphura CRM Team</p>
            <hr style="border: none; border-top: 1px solid #ddd; margin: 20px 0;">
            <p style="color: #999; font-size: 12px; text-align: center;">
              Graphura CRM | Create Bold. Edit Smart. Design Loud.
            </p>
    ...
    logger.info(`Contact acknowledgement email sent to ${email}`);
    return {
      success: true,
    ...
    const details =
      error.response?.data?.message || error.response?.data || error.message;
    logger.error("Failed to send contact acknowledgement email", details);
    throw new Error(
      typeof details === "string"
        ? details
        : "Unable to send contact acknowledgement email.",
    );
  }
};
