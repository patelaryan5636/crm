...
import {
  ShieldCheck, Eye, EyeOff, CreditCard, Hash,
  Building2, Users, Zap, BadgeCheck, Mail,
} from "lucide-react";
...
const FloatingBackground = () => (
  <div className="absolute inset-0 z-0 overflow-hidden opacity-5 pointer-events-none">
    <ShieldCheck className="w-20 h-20 text-slate-900 absolute top-10 left-10 animate-[spin_25s_linear_infinite]"    strokeWidth={1} />
    <CreditCard  className="w-24 h-24 text-slate-900 absolute top-20 right-20 animate-[pulse_4s_ease-in-out_infinite]" strokeWidth={1} />
    <Building2   className="w-16 h-16 text-slate-900 absolute bottom-32 left-40 animate-[spin_22s_linear_infinite]"  strokeWidth={1} />
    <Users       className="w-20 h-20 text-slate-900 absolute -bottom-10 right-40 animate-[pulse_5s_ease-in-out_infinite]" strokeWidth={1} />
    <Zap         className="w-28 h-28 text-slate-900 absolute top-[40%] left-[20%] animate-[spin_18s_linear_infinite]" strokeWidth={1} />
    <BadgeCheck  className="w-16 h-16 text-slate-900 absolute bottom-20 right-28 animate-[pulse_3s_ease-in-out_infinite]" strokeWidth={1} />
  </div>
);
        ...
        <div key={label} className="flex items-center gap-2">
          <div className="flex flex-col items-center gap-1">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black transition-all duration-300 ${
              done ? "bg-emerald-500 text-white" : active ? "bg-[#2a465a] text-white" : "bg-slate-200 text-slate-400"
            }`}>
              {done ? "✓" : i + 1}
            </div>
            <span className={`text-[10px] font-bold uppercase tracking-widest ${
              active ? "text-[#2a465a]" : done ? "text-emerald-500" : "text-slate-400"
            }`}>
              {label}
            </span>
          </div>
          {i < STEPS.length - 1 && (
            <div className={`h-[2px] w-10 rounded-full mb-4 transition-all duration-500 ${done ? "bg-emerald-400" : "bg-slate-200"}`} />
          )}
        </div>
      );
    })}
...
const strengthColor = (s) =>
  s === 1 ? "bg-red-400" : s === 2 ? "bg-yellow-400" : s === 3 ? "bg-blue-400" : "bg-emerald-400";
...
export default function DepartmentWorkspace() {
  ...
  const statusCls =
    ...
    : status.type === "alert" ? "border-yellow-200 bg-yellow-50 text-yellow-800"
      ...
      <div className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl flex overflow-hidden z-10 min-h-[600px]">
        ...
        <div className="hidden lg:flex w-5/12 bg-slate-50 p-12 flex-col justify-between border-r border-slate-100">
          <div>
            ...
            <p className="text-slate-500 text-sm leading-relaxed">
              Secure your account and add your bank details to get started with the Graphura CRM platform.
            </p>
          ...
          <div className="flex gap-2">
            <button className="text-[11px] font-bold px-4 py-2 bg-white rounded-full border border-slate-200 text-slate-600 hover:bg-slate-100">Terms of Service</button>
            <button className="text-[11px] font-bold px-4 py-2 bg-white rounded-full border border-slate-200 text-slate-600 hover:bg-slate-100">Privacy Policy</button>
          </div>
        </div>
        ...
        <div className="w-full lg:w-7/12 p-8 md:p-14 flex flex-col justify-center">
          <div className="max-w-md mx-auto w-full">
            ...
            {status.message && (
              <div className={`rounded-2xl px-4 py-3 text-sm mb-5 border ${statusCls}`}>
                {status.message}
              </div>
            )}
              ...
              {step === 0 && (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    {password && (
                      <div className="flex gap-1 mt-1.5 px-1">
                        {[1, 2, 3, 4].map((n) => (
                          <div key={n} className={`h-1 flex-1 rounded-full transition-all ${n <= strength ? strengthColor(strength) : "bg-slate-200"}`} />
                        ))}
                      </div>
                    )}
                    {errors.password && <p className="text-xs text-rose-600 px-1">{errors.password}</p>}
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    {errors.confirmPassword && <p className="text-xs text-rose-600 px-1">{errors.confirmPassword}</p>}
                  </div>
                </>
              )}
                ...
                <>
                  <div className="space-y-1">
                    <DataField
                      ...
                      icon={Building2}
                      value={bankName}
                      onChange={(e) => { setBankName(e.target.value); setErrors((er) => ({ ...er, bankName: "" })); }}
                      size={12}
                    />
                    {errors.bankName && <p className="text-xs text-rose-600 px-1">{errors.bankName}</p>}
                  </div>
                  ...
                  <div className="space-y-1">
                    <DataField
                      ...
                      icon={CreditCard}
                      value={accountNumber}
                      onChange={(e) => { setAccountNumber(e.target.value.replace(/\D/g, "")); setErrors((er) => ({ ...er, accountNumber: "" })); }}
                      size={12}
                    />
                    {errors.accountNumber && <p className="text-xs text-rose-600 px-1">{errors.accountNumber}</p>}
                  </div>
                  ...
                  <div className="space-y-1">
                    <DataField
                      ...
                      onChange={(e) => { setConfirmAccountNumber(e.target.value.replace(/\D/g, "")); setErrors((er) => ({ ...er, confirmAccountNumber: "" })); }}
                      size={12}
                    />
                    {errors.confirmAccountNumber && <p className="text-xs text-rose-600 px-1">{errors.confirmAccountNumber}</p>}
                  </div>
                  ...
                  <div className="space-y-1">
                    <DataField
                      ...
                      icon={Hash}
                      value={ifscCode}
                      onChange={(e) => { setIfscCode(e.target.value.toUpperCase()); setErrors((er) => ({ ...er, ifscCode: "" })); }}
                      size={12}
                    />
                    {errors.ifscCode && <p className="text-xs text-rose-600 px-1">{errors.ifscCode}</p>}
                  </div>
                </>
              )}
              ...
              {step === 2 && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center space-y-3">
                  ...
                  <p className="text-sm text-emerald-700">
                    Your password and bank details have been saved. Click <strong>Finish</strong> to go to your dashboard.
                  </p>
                  <div className="pt-2 text-left rounded-2xl border border-slate-100 bg-white p-4 space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span className="text-slate-500">Password</span><span className="font-semibold text-emerald-600">✓ Set</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Bank Name</span><span className="font-semibold text-[#2a465a]">{bankName}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Account Number</span><span className="font-semibold text-[#2a465a]">{"•".repeat(Math.max(0, accountNumber.length - 4)) + accountNumber.slice(-4)}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">IFSC Code</span><span className="font-semibold text-[#2a465a]">{ifscCode}</span></div>
                  </div>
                </div>
              )}
