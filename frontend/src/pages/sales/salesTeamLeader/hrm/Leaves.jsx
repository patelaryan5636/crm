...
import { leaveRequests as TEAM_LEAVE_REQUESTS, LEAVE_TYPES } from "../myTeam/teamStore";
...
// Manager uses long-form leave types ("Sick Leave"); MOCK_LEAVES_INIT uses short form ("Sick").
const expandLeaveType = (t) => (t.endsWith(" Leave") ? t : `${t} Leave`);

const today = () => new Date().toISOString().split("T")[0];

// ─── Normalize MOCK_LEAVES_INIT → row shape used in the My Leaves table ─────
const toMyRow = (l) => ({
  ...l,
  type:      expandLeaveType(l.type),
...
const MY_COLS = [
  ...
  { key: "status",    label: "Status" }, // auto-renders
];
...
export default function Leaves() {
  // ── State: TL's own leaves + executives' leave requests ─────────────────
  const [myLeaves, setMyLeaves] = useState(() => MOCK_LEAVES_INIT.map(toMyRow));
  const [teamReqs, setTeamReqs] = useState(TEAM_LEAVE_REQUESTS);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        actions={[
          {
            ...
            onClick: (row) => { setMyView(myLeaves.find((l) => l.id === row.id)); openModal("tl-hrm-myleave-view"); },
          },
          {
      ...
      {/* ── Pending Leaves table (executives) ────────────────────────────── */}
      <DataTable
        ...
        actions={[
          {
            ...
            onClick: (row) => { setPendingView(teamReqs.find((r) => r.id === row.id)); openModal("tl-hrm-pending-view"); },
          },
          {
            icon: <BadgeCheck size={15} />, tooltip: "Approve", variant: "primary",
            onClick: (row) => setExecStatus(row, "Approved"),
          },
          {
            icon: <Ban size={15} />, tooltip: "Reject", variant: "danger",
            onClick: (row) => setExecStatus(row, "Rejected"),
          },
        ]}
      />

      {/* ── Leave History table (executives — actioned) ──────────────────── */}
      <DataTable
        title="Leave History"
        ...
        bulkActions={[
          {
            ...
            onClick: (selected) => {
              ...
              setTeamReqs((prev) => prev.filter((r) => !ids.has(r.id) || r.status === "Pending"));
        ...
        actions={[
          {
            icon: <Eye size={15} />, tooltip: "View Details", variant: "ghost",
            onClick: (row) => { setHistoryView(teamReqs.find((r) => r.id === row.id)); openModal("tl-hrm-history-view"); },
          },
        ]}
      />
      ...
      {/* ── My Leave View modal ──────────────────────────────────────────── */}
      <Modal id="tl-hrm-myleave-view" title="My Leave Details" size="md">
        {myView && (
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              {myView.status === "Pending" && (
                <Button text="Cancel Application" variant="danger" size={4}
                  onClick={() => { cancelMyLeave(myView); closeModal("tl-hrm-myleave-view"); }} />
              )}
              <Button text="Close" variant="ghost" size={3} onClick={() => closeModal("tl-hrm-myleave-view")} />
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal id="tl-hrm-pending-view" title="Leave Request Details" size="md">
        {pendingView && (
          <div className="flex flex-col gap-4">
            <ModalProfile
              name={pendingView.name}
              subtitle={`Sales Executive · ${pendingView.type}`}
            ...
            {pendingView.status === "Pending" && (
              <div className="flex gap-3 pt-2 border-t border-slate-100">
                <Button text="Approve" variant="primary" size={6}
                  onClick={() => { setExecStatus(pendingView, "Approved"); closeModal("tl-hrm-pending-view"); }} />
                <Button text="Reject" variant="danger" size={6}
                  onClick={() => { setExecStatus(pendingView, "Rejected"); closeModal("tl-hrm-pending-view"); }} />
              </div>
            )}
            <div className="flex justify-end">
      ...
      <Modal id="tl-hrm-history-view" title="Leave Record" size="md">
        {historyView && (
          <div className="flex flex-col gap-4">
            <ModalProfile
              name={historyView.name}
              subtitle={`Sales Executive · ${historyView.status}`}
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100">
              <Button text="Close" variant="primary" size={3} onClick={() => closeModal("tl-hrm-history-view")} />
            </div>
          </div>
        )}
      </Modal>
